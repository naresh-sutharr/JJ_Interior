...
function AdminLogin() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      let { error: signInError } = await supabase.auth.signInWithPassword({ email: cleanEmail, password });
      ...
      if (signInError && signInError.message.includes("Invalid login credentials")) {
        ...
        const { error: retryError } = await supabase.auth.signInWithPassword({ email: cleanEmail, password });
        if (retryError) {
